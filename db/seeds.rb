...
tests = Test.create([
    ...
    {title: "Хоккей", level: "1", category_id: categories[0].id, author_id : users[0].id},
    {title: "Наполеон", level: "3", category_id: categories[1].id, author_id : users[1].id},
    {title: "Вторая мировая война", level: "1", category_id: categories[1].id, author_id : users[1].id},
...
questions = Question.create([
    ...
    {body: "В каком году родился Наполеон Бонапарт?", test_id: tests[2].id},
    {body: "По скольки номинациям вручается премия Оскар?", test_id: tests[4].id}
])
...
answers = Answer.create([
    ...
    {text: "24", correct: false, question_id: questions[2].id},
])
